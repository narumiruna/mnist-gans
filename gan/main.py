...
def train(epoch):
    for i, (x, _) in enumerate(mnist_loader):
        ...
        ones = autograd.Variable(torch.ones(x.size(0)))
        zeros = autograd.Variable(torch.zeros(x.size(0)))
        ...
        fake = net_g(z).detach()
        loss_d = bce(net_d(x), ones) + bce(net_d(fake), zeros)

...
def train(epoch):
    for i, (x, _) in enumerate(mnist_loader):
        ...
        epsilon = autograd.Variable(torch.rand(x.size(0), 1, 1, 1))
        grad_outputs = autograd.Variable(torch.ones(x.size(0)))

        if use_cuda:
            epsilon = epsilon.cuda()
            grad_outputs = grad_outputs.cuda()

        interpolates = epsilon * x + (1 - epsilon) * fake
        interpolates.requires_grad = True
        gradients = autograd.grad(net_d(interpolates),
                                interpolates,
                                grad_outputs=grad_outputs,
                                create_graph=True)[0]

        gradient_penalty = (gradients.view(x.size(0), -1).norm(2, dim=1) - 1).pow(2)

        loss_d = -net_d(x).mean() + net_d(fake).mean() + args.c * gradient_penalty.mean()
